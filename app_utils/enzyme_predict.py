...
def enzyme_predict(seq, client, model):
    # Valid aminoacids
    valid_aac = 'ACDEFGHIKLMNPQRSTVWY'
    # Check if sequence is string
    if isinstance(seq, str):
        # Check if sequence have only valid aminoacids
        if all(aac in valid_aac for aac in seq):
            # Encode sequence
            ...
            result = 'Sequence have not valid aminoacids'
            return result
    else:
        result = 'Sequence is not string'
        return result